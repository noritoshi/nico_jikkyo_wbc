...
- 音声入力（Deepgram API）— マイクで喋ってコメント投稿。自動投稿/手動入力モード切替対応
...
2. コメント入力欄の「**音声**」ボタンをクリックしてマイクを有効化
3. 喋った内容が自動で音声認識されコメントとして投稿されます
4. 「**自動**」ボタンをクリックすると「**手動**」モードに切替（入力欄にテキスト表示 → Enter で送信）
5. 「**停止**」ボタンで音声入力を終了
