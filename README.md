...
残念ながら2026年WBCで日本が準々決勝敗退してしまいましたが、手に汗握る熱い試合を見せてくれた選手、監督、スタッフのみなさんありがとう！お疲れ様でした。今後の侍ジャパンの活躍を楽しみにしています！
...
- 音声入力（Deepgram API）— マイクで喋ってコメント投稿。WebSocket ストリーミングによるリアルタイム認識、自動投稿/手動入力モード切替対応
- 音声認識の AI 後処理 — 同音異義語のローカル辞書修正（早い→速い 等）、Gemini API による文脈ベースの誤認識修正・品質チェック、意味不明なノイズの自動却下
- 音声認識キーワード自動取得 — 対戦チーム名を入力すると AI が選手名を Web 検索で取得し、音声認識の精度を向上。カスタムキーワードの手動追加・個別削除にも対応
...
- 実況ダッシュボード（右サイドパネル）— コメント推移グラフ・AI 概要・ユーザー一覧を同時表示
  - コメント推移グラフ: 直近10分間のコメント頻度を棒グラフで可視化（盛り上がりが一目でわかる）
  - AI 概要: 場の雰囲気・話題のトピックを自動分析（3分間隔で自動更新）
  - ユーザー一覧: 各ユーザーのコメント数・傾向カテゴリ（解説/ツッコミ/応援/悲嘆/盛り上げ/予想/雑談）をリアルタイム表示（クリックで全コメント展開）
- プレミアム会員対応 — 上固定・下固定コメント、下積みコメントアート
- 自分のコメントを黄色枠で強調表示
- コメントの一時非表示/表示切り替え
- Terminal Minimal Dark テーマ — ダークネイビー基調にシアン/レッドアクセントのモダン UI
...
1. ポップアップの「Voice Settings」で Deepgram API Key を設定（[Deepgram Console](https://console.deepgram.com/) で取得、従量課金）
2. （推奨）対戦チーム名を入力して「**選手取得**」をクリック — AI が選手名を検索し、音声認識のキーワードに自動設定
3. コメント入力欄の「**音声**」ボタンをクリックしてマイクを有効化
4. 喋った内容がリアルタイムで音声認識されコメントとして投稿されます
5. 「**自動**」ボタンをクリックすると「**手動**」モードに切替（入力欄にテキスト表示 → Enter で送信）
6. 「**停止**」ボタンで音声入力を終了
...
- Deepgram Nova-3 WebSocket API（音声認識、keytermプロンプティング）
